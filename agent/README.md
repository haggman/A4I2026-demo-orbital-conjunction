...
| **Agent Runtime code sandbox** | ADK's `AgentEngineSandboxCodeExecutor`; the model writes Python, the sandbox runs it | Orbital what-ifs—*"what's the smallest burn that gets us clear?"*—run somewhere isolated, not in the agent's process |
...
agent/
  cymbal_ops/                 the ADK agent (adk web / adk deploy point here)
    ...
    tools.py                  assess_conjunction, maneuver_cost, build_assessment
