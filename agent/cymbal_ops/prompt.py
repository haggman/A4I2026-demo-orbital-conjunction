...
_EXAMPLE = """      import orbit_whatif as ow
      ...
      for t in ["2026-09-25T06:00:00Z", "2026-09-25T10:00:00Z"]:
          print(t, ow.smallest_burn(case, t))                           # prograde by default
...
if config.CODE_PATH == "executor":
    _CODE = f"""- Code execution: write a ```python block in your reply and it runs in an isolated Agent Runtime sandbox. Use it
  for orbital what-ifs, and only through the orbit_whatif module the case file is made for:
{_EXAMPLE}
...
"""
    ...
    _CODE = f"""- run_in_sandbox(code): runs Python in an isolated Agent Runtime sandbox and returns what it printed. Use it for
  orbital what-ifs, and only through the orbit_whatif module the case file is made for, for example:
{_EXAMPLE}
  Pass the code as the `code` argument; never write it in your reply. Keep it short, print the results, and put
  every burn time you need in ONE call (a loop is fine). After it returns, call maneuver_cost for each burn you
