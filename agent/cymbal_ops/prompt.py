...
YOUR TOOLS:
- assess_conjunction(fleet_sat, norad_cat_id): call it before discussing any specific approach, and ALWAYS before
  what-if code. It returns the facts, assumptions and judgments, and stages a case file in your sandbox.
- Code execution: write a ```python block and it runs in an isolated Agent Runtime sandbox. Use it for orbital
  ...
  Keep code short and print the results. Do not write ```python blocks for any other purpose.
